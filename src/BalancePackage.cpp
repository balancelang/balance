...
bool BalancePackage::compile() {
    ...
    BalanceModule * bmodule = new BalanceModule(this->entrypoint, true);

    this->buildDependencyTree(bmodule);
    this->compileModules(this->modules);
    this->writePackageToBinary(this->entrypoint);
}

bool BalancePackage::compileBuiltins() {
    createBuiltins();
    this->addBuiltinSource("standard-library/range.bl");
    this->compileModules(this->builtinModules);
}

bool BalancePackage::addBuiltinSource(std::string path) {
    BalanceModule * bmodule = new BalanceModule(path, false);
    currentPackage->builtinModules[path] = bmodule;
}
...
// bool BalancePackage::compile() {
//     BalanceModule * bmodule = new BalanceModule(this->entrypoint, true);
//     BalanceSource * bsource = new BalanceSource();
//     bsource->filePath = this->entrypoint;
//     bmodule->sources = { bsource };
//     compileModule(bmodule);
//     return true;
// }
// {
...
void BalancePackage::writePackageToBinary(std::string entrypointName)
{
    ...
    writeModuleToBinary(this->builtinModules["builtins"]);
