...
void createVariablesTestInt() {
    ...
    std::string expected = "(root (rootBlock (lineStatement (statement (assignment (newAssignment var a = (expression (literal (numericLiteral 555)))))) <EOF>)))";
...
void createVariablesTestString() {
    ...
    std::string actual = programToString(program);
    std::string expected = "(root (rootBlock (lineStatement (statement (assignment (newAssignment var a = (expression (literal (stringLiteral Hello world!)))))) <EOF>)))";
    assertEqual(expected, actual, program);
}
...
void createVariablesTestBool() {
    ...
    std::string actual = programToString(program);
    std::string expected = "(root (rootBlock (lineStatement (statement (assignment (newAssignment var a = (expression (literal (booleanLiteral true)))))) <EOF>)))";
    assertEqual(expected, actual, program);
}
...
void createVariablesTestFloat() {
    ...
    std::string actual = programToString(program);
    std::string expected = "(root (rootBlock (lineStatement (statement (assignment (newAssignment var a = (expression (literal (doubleLiteral 5.3)))))) <EOF>)))";
    assertEqual(expected, actual, program);
}
...
void createVariablesTestLambda() {
    ...
    std::string actual = programToString(program);
    std::string expected = "(root (rootBlock (lineStatement (statement (assignment (newAssignment var myFunc = (expression (lambda ( parameterList ) -> { functionBlock }))))) <EOF>)))";
    assertEqual(expected, actual, program);
}
